...
/**
 * Ajustes clave:
 * - El modal usa clases `fb-modal-dashboard` / `fb-modal-map` para el `padding-top`,
 *   tal como sugeriste (misma técnica que te funcionó antes).
 * - Mantengo `variant` para aplicar paddings distintos en la barra (.fb-dashboard / .fb-map).
 * - "Aplicar filtros" aplica; "Limpiar" resetea y aplica vacío inmediatamente.
 * - Incluye filtro "Estado de venta" con píldoras.
 * - Al abrir, el body muestra solo Estado/Sub-estado/Calle (resto al scrollear).
 */
export default function FilterBar({
  ...
  const ESTADO_VENTA = ["INICIADA","CON_BOLETO","ESCRITURA_PROGRAMADA","ESCRITURADO"];
  ...
  // Refs para controlar el alto visible del body = alto del “TOP”
  ...
  const activeCount = useMemo(() =>
    (q?1:0) + estado.length + subestado.length + calle.length + estadoVenta.length +
    (frente.min!==0||frente.max!==100?1:0) +
    (fondo .min!==0||fondo .max!==100?1:0) +
    (sup   .min!==0||sup   .max!==5000?1:0) +
    (precio.min!==0||precio.max!==300000?1:0) +
    (deudor===null?0:1),
  [q,estado,subestado,calle,estadoVenta,frente,fondo,sup,precio,deudor]);

  // Al abrir: bloquear scroll y fijar alto visible del body al alto del TOP
  useEffect(() => {
    if (!open) return;
  ...
  // Aplica filtros (solo al tocar botón)
  const applyFilters = () => {
    onParamsChange?.({
      q, estado, subestado, calle, estadoVenta,
  ...
  // Limpia todo y aplica vacío
  const clear = () => {
    setQ(""); setEstado([]); setSubestado([]); setCalle([]); setEstadoVenta([]);
  ...
  // Modal (portal) – ahora con clase por variante para el padding-top
              ...
              <div className="fb-options">
                {ESTADOS.map(v => (
                  <button key={v}
                    ...
                    onClick={() => setEstado(prev => prev.includes(v) ? prev.filter(x=>x!==v) : [...prev, v])}
                  >{nice(v)}</button>
                ))}
              </div>
            </section>
              ...
              <div className="fb-options">
                {SUBESTADOS.map(v => (
                  <button key={v}
                    ...
                    onClick={() => setSubestado(prev => prev.includes(v) ? prev.filter(x=>x!==v) : [...prev, v])}
                  >{nice(v)}</button>
                ))}
              </div>
            </section>
              ...
              <div className="fb-options fb-grid">
                {CALLES.map(v => (
                  <button key={v}
                    ...
                    onClick={() => setCalle(prev => prev.includes(v) ? prev.filter(x=>x!==v) : [...prev, v])}
                  >{nice(v)}</button>
                ))}
              </div>
            </section>
          ...
          <div className="fb-body-rest">
            <section className="fb-section">
              ...
              <div className="fb-options">
                {ESTADO_VENTA.map(v => (
                  <button key={v}
                    className={`fb-pill ${estadoVenta.includes(v) ? "is-checked" : ""}`}
                    onClick={() => setEstadoVenta(prev => prev.includes(v) ? prev.filter(x=>x!==v) : [...prev, v])}
                  >{nice(v)}</button>
                ))}
              </div>
            </section>
